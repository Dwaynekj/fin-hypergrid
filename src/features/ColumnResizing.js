...
var ColumnResizing = Feature.extend("ColumnResizing", {
  ...
  overAreaDivider: function(grid, event) {
    ...
    return (
      (event.detail.gridCell.x !== leftMostColumnIndex &&
        event.detail.mousePoint.x <= 3) ||
  ...
  handleMouseDown: function(grid, event) {
    if (event.detail.isHeaderRow && this.overAreaDivider(grid, event)) {
      if (event.detail.mousePoint.x <= 3) {
        ...
        this.dragStartWidth =
          grid.renderer.visibleColumns[visibleColIndex].width;
      } else {
  ...
  handleMouseMove: function(grid, event) {
    if (!this.dragColumn) {
      ...
      this.cursor =
        event.detail.isHeaderRow && this.overAreaDivider(grid, event)
          ? this.getCursorName()
          : null;
    }
  },
  ...
  handleDoubleClick: function(grid, event) {
    if (event.isHeaderRow && this.overAreaDivider(grid, event)) {
      ...
      setTimeout(function() {
        // do after next render, which measures text now that auto-sizing is on
